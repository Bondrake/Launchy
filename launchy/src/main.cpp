    ...
    condensedTempIcon(NULL)
{
    ...
    connect(&iconExtractor, SIGNAL(iconExtracted(CatItem, QIcon)), this, SLOT(iconExtracted(CatItem, QIcon)), Qt::BlockingQueuedConnection);
...
void LaunchyWidget::doCtrlEnter()
{
    hideAlternatives();
    ...
    if ((inputData.count() > 0 && searchResults.count() > 0) || inputData.count() > 1)
    {
        ...
        qDebug() << "Opening " << fi.absolutePath();
        runProgram(fi.absolutePath(), "");
        hideLaunchy();
    }
    else
    {
...
void LaunchyWidget::iconExtracted(CatItem item, QIcon icon)
{
    ...
    if (item.id == -1)
    {
        ...
        if (item.fullPath == outputItem.fullPath)
        {
